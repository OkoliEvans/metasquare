...
contract TicketFactory {
    ...
    function createEvent(
        ...
        if (_regStartTime <= 0 || _regEndTime <= _regStartTime)
            revert("Invalid reg. start or end time");
        if (zeroHash == keccak256(abi.encode(_eventUri)))
            revert Invalid_Event_Uri();
        ...
        Poap poap = new Poap(address(iticketing));
    ...
    function showTotalEventAddresses()
        external
        view
